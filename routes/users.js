...
router.post("/postadd",checkUser,upload.single("image"),
...
router.get("/postlist", checkUser, async function (req,res){
  const posts = await Post.find({ author: req.session.user.id}).populate("author");
  console.log(posts);
...
router.get("/postdetail/:id", checkUser,async function (req, res){
  ...
  const comments=await Comment.find({post:req.params.id}).populate("commenter","name");
  console.log(post);
